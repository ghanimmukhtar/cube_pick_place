...
public:
    ...
    void init(){
        ...
        _mode = static_cast<string>(_parameters["mode"]);
        ...
        /// If the mode is manual then you should enter manually the picking and placing positions
        if(_mode == "manual"){
            ROS_INFO_STREAM("PICK_PLACE : INIT : The mode is manual, you should enter position for picking and position for placing in the topic pick_place_pose ....");
...
private:
    NodeHandle _node;
    string _table_object_name = "Grasp_Table", _grasp_object_name = "Grasp_Object", _arm_group = "arm", _gripper_group = "gripper", _arm, _gripper, _mode;
